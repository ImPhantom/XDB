...
using System.Net;
using System.Text;
using Phantom.CSourceQuery.Models;
using Phantom.CSourceQuery.Network;
...
namespace Phantom.CSourceQuery
{
    ...
    {
        public async Task<SourceServer> QueryServer(string ip, int port)
        {
            var endPoint = await (new IPHelper()).GetIPEndPointFromHostName(ip, port, true);
            return QueryServer(endPoint);
        }

        public SourceServer QueryServer(EndPoint ip_end)
        {
            //Create a new empty server info object
            SourceServer info = new SourceServer();
            ...
            try
            {
                ...
                buf = SocketHelper.getInfo(ip_end, requestPacket);
            }
            ...
            if ((char)buf[i] == 'l')
                info.Dedicated = SourceServer.DedicatedType.LISTEN;
            else if ((char)buf[i] == 'd')
                info.Dedicated = SourceServer.DedicatedType.DEDICATED;
            else if ((char)buf[i] == 'p')
                info.Dedicated = SourceServer.DedicatedType.SOURCETV;
            ...
            //Get the OS type
            if ((char)buf[i] == 'l')
                info.OS = SourceServer.OSType.LINUX;
            else if ((char)buf[i] == 'w')
                info.OS = SourceServer.OSType.WINDOWS;
        ...
        public async Task<List<PlayerDetails>> QueryPlayers(string ip, int port)
        {
            IPEndPoint endPoint = await (new IPHelper()).GetIPEndPointFromHostName(ip, port, true);
            return QueryPlayers(endPoint);
        }

        public List<PlayerDetails> QueryPlayers(IPEndPoint ip_end)
        {
            ...
            try
            {
                ...
                buf = SocketHelper.getInfo(ip_end, challenge);
            }
            catch (SourceQueryException e)
            {
                throw e;
            ...
            try
            {
                ...
                buf = SocketHelper.getInfo(ip_end, requestPlayer);
            }
